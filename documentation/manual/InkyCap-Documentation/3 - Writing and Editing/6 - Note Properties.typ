...
Properties make your notes _findable_ and _organizable_. Once a note knows its own date, tags, and collection, InkyCap can gather it into the right #wikilink("2 - Collections"), retrieve it in searches, and let other tools read it too.
...
The rows appear in the same order they're written in the note.

When you open a large notebox, InkyCap indexes it in the background. If InkyCap has seen the note before and it hasn't changed since, its properties appear immediately from the cache. A note that is new or has changed will fill in its properties as soon as indexing is done.
...
- The *property name*, shown as written.
...
- *title* is a name for the note (text), which can differ from the filename.
- *description* is for use as a summary (text). This is also what shows up in tables of contents and previews.
...
- *date* and *due* are calendar dates, chosen with a date picker and useful for things such as the Agenda view.
- *task* is a checkbox for turning a note into something actionable (and inclusion in the Agenda view).
...
The editor for each row matches its type:
...
- *Checkbox* fields are a single tick box; the property's name carries the meaning.
- *Date* and *Date & time* fields open a calendar picker.
- *List* fields show your values as multi-select pills. Click the row, or press *Enter* while it has the focus, to open a list of the values already used for that property elsewhere in your notebox. Type in the *Filter or add new…* box to narrow the list; the arrow keys (and *Home* and *End*) move the highlight, and *Enter* adds or removes the highlighted value, so a few letters and *Enter* is usually enough. If what you typed isn't in the list yet, an `+ Add "..."` row appears; choose it to create the value. *Escape* closes the list and returns you to the pills, and *Backspace* in an empty filter box removes the last pill.
- The *collection* field has its own picker: click it to explicitely include the note in particular collections you have created. If you haven't made any yet, it'll say "No collections defined".
...
+ If you're creating a *brand-new* field, a short type menu appears so you can choose what kind of value it holds: Checkbox, Date, Date & time, List, Comma list, Number, or Text.
...
#callout("caution")[A list property differs from a comma list because it will provide you with an interface to select from within all of the options you've created. A comma list is a text field in which each item is separated by a comma. A comma list is likely most valuable in the case that you do not want to use a large list interface but still want to incorporate multiple pre-existing values.]
...
#callout("tip", title: "For Typst users")[`#note(...)` comes from the bundled `inkycap-notebox` package and emits a `#metadata(...)` dict tagged with the stable `<inkycap-note>` label (at most one per file). That means any Typst toolchain can `query()` the label and read the same dictionary. Your metadata is plain, portable Typst (text), not a proprietary side-channel.
...
Because properties are queryable Typst metadata under a stable label, InkyCap can index them and use them to provide features you'll meet elsewhere:
