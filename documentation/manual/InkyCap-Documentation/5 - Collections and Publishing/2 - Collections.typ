...
#callout("note")[Collections are stored automatically inside your notebox and managed for you. You do not need to find or organize the collection files by hand. Create them from the *Collections* section in the left sidebar with the *"+"* button, give the collection a name, and a starter table appears.]

#callout("tip", title: "Technical background")[A collection is a single YAML file with the `.collection` extension, kept in the reserved `.inkycap/collections/` folder at your notebox root. The file holds a global filter group, one or more view definitions, optional style overrides, and book-export metadata; but you edit all of it through the collection's panels, never as text.]

== How notes join a collection

Membership is decided entirely by *filters*. There is no separate "add to collection" list to maintain. A note shows up as part of the collection when it matches the collection's rules. There are two complementary ways to write those rules, and you can mix them freely.
...
#callout("tip", title: "For Typst users")[The property lives in the note's `#note(...)` call as a list:
  ```typ
...
- *equals* / *not equals*
- *less than* / *at most* / *greater than* / *at least* (these compare numbers, or dates written as `YYYY-MM-DD`, in order)
...
#callout("important")[An *empty filter matches every note*. A brand-new collection with no rules yet will list your whole notebox until you narrow it down. ]

#callout("note")[When a rule compares a _list_ property (like `collection` or `tags`) against a single value, it tests *membership* ("is this value in the list?"). So a "contains" rule on `tags` finds notes that carry that tag, and the collection-name rule finds notes that list that collection.]

#callout("tip", title: "For Typst users")[Filter rules are stored as small expression strings you can also hand-author. Property references can be bare (`title`), bracketed for awkward keys (`note["due-date"]`), file metadata (`file.folder`), or a self-reference to the collection file (`this.file.name`). `contains` is a case-sensitive substring match, an absent property counts as empty, and a malformed expression *fails closed*; it's simply skipped rather than matching by accident. The default filter keeps a collection out of its own results with `file.name != this.file.name`.]
...
*Opening notes.* The file-name cell is a link. Click it to open the note, or Ctrl/Cmd-click (or middle-click) to open it in a new tab. Each row also carries a small pen icon that opens the note straight in the visual editor (Ctrl/Cmd-click it for a new tab). Right-click any row for *Open note*, *Open in new tab*, or *Export note…*.
...
#callout("note")[Columns drawn from file details (the folder, path, dates, size) are *read-only*, since they come from the filesystem itself. Everything you authored as a property is editable.]

*Choosing columns.* The *Columns* button opens a checklist of every available property; tick the ones you want to appear as columns. Drag column headers to reorder them, and drag the right edge of a header to make a column wider or narrower (the widths are saved with the view). The file-name column is always present.
...
#callout("tip")[Friendly labels keep headers readable. File details show as "Folder Name", "Modified Date", "File Extension", and so on, even though InkyCap stores their precise technical keys behind the scenes.]
...
Clicking an item opens its note (Ctrl/Cmd or middle-click for a new tab). If a collection has no tasks or dated items, the view says so plainly.

The view has the same *Task List* and *Date* filters as the sidebar Agenda, including the *Due today* shortcut and dates set as a number of days from today. Two differences: the filters you set here are not remembered between visits, and there is no *Bookmark current view*, because the collection's own saved views play that role. For the full picture of how tasks, due dates, and the filters work, see #wikilink("3 - Agenda, Tasks, and Dates").
