...
After starting InkyCap the first thing you'll see is a screen titled *"Open a notebox to continue."* InkyCap requires an active #highlight[notebox] to do anything.
...
The folder is now your notebox. Until you open or create a note, the editor area shows an empty tab with four clickable hints that double as reminders of the most useful shortcuts:

- *Open a file by name: Ctrl + O*
- *Create a note: Ctrl + N*
- *Access commands quickly: Ctrl + P*
- *How to write basic Typst markup: F1*, which opens the Help panel on its Typst markup cheat-sheet.

Click any hint to do that thing right away, or press the keys shown.
...
Creating a fresh notebox is the common path, but you can also join a notebox someone else has shared to collaborate with you:

- *Clone from remote* lets you join a collaborative notebox by its git address. 
- *Import package* lets you join a notebox that a collaborator handed to you (for example, offline by USB key or as an e-mail attachment) as a single package file.
...
A new note opens from a scaffold and template, already containing a few helpful fields and a heading, ready for you to type. 

#callout("tip")[ There's no Save button or save shortcut. InkyCap saves your work automatically as you type, and writes each file so you never end up with a half-saved note. ]
...
#callout("important")[InkyCap reads Typst markup. If you are familiar with Markdown, you will need to make small changes to habits like `**bold**` or `# Heading` because they will not be styled properly.]

== Step 3 — Link notes together

Linking is the heart of how you connect your ideas in InkyCap. To link from one note to another, type two square brackets and continue typing the words for a note to link to:
...
Every link you make is automatically two-way. When you link from note A to note B, note B gains a _backlink_ pointing back to A. Over time these connections graph your thinking and means you have an automatic organization of all of the information related to the topic of a note. See #wikilink("4 - Links and Backlinks") to go deeper.
...
InkyCap can make backups of your whole notebox while you work. Backups are switched on by default, but they only start running once you tell InkyCap _where_ to put them.
