...
#callout("warning")[Extensions are *experimental*. The feature works, but it is early, and the way you set things up may change in future versions. In the app you will see the notice "This feature is experimental and might not function perfectly." Treat what is here as a solid foundation, not a finished, locked-in system.]
...
#callout("note")[The Extensions tab in Settings covers one specific kind of extension: connecting *external programs* to InkyCap. The other ways to extend the app (described further down) are file-based and have no settings screen. They are aimed at more technical users who are comfortable editing a small text file.]

== Connecting external tools

The Extensions tab is a bridge to *external tools* (any program already on your computer that you trust). The idea is relatively simplistic: InkyCap hands some of your text to the program, the program does its work and prints a result, and InkyCap takes that result back. This is how you might wire up a grammar checker like #link("https://languagetool.org")[LanguageTool], a style linter, an AI rewriting helper, dictation clean-up, or almost anything else.

#callout("important")[InkyCap ships with *no* built-in extensions and runs *nothing* on its own. An extension tool only ever runs because you explicitly added it and pointed InkyCap at the program's location.]
...
Each tool card lets you decide how text flows in and out.
...
#callout("tip")[Pick "Command palette" + "Replace the selection" for editing helpers (grammar, rewriting), and "Show it in a side panel" for tools that produce a report you want to keep reading. If a tool just answers a quick question, "Show a temporary message" keeps things tidy.]

=== Things to know about safety and limits

- *Privacy stays in your hands.* InkyCap does not send your notes anywhere on its own. If a tool you set up talks to a network service, that is your choice; your note's text goes wherever you pointed it.
...
- *No surprises.* InkyCap runs the program you registered, identified internally and it does not route commands through a shell.

#callout("tip", title: "Details")[This is a plain Unix-pipe bridge: InkyCap writes to the program's *stdin* and applies its *stdout*; *stderr* is discarded and a non-zero exit code is reported as a failure. *Arguments* are one per line and passed as a vector (never a shell string), with three literal placeholders substituted verbatim:
  ...
  For a program that reads its input from arguments rather than a stream, set *Send to the tool* to "Nothing" and put `$INKYCAP_SELECTION` (or `$INKYCAP_FILE`) in *Arguments*. Stripping to plain text uses an AST-based extractor that follows the Typst parser. Advanced users can also edit `external_tools.tools` directly in `settings.json`.]
...
#callout("tip", title: "Details")[The two folders InkyCap reads are `plugins/` inside its configuration folder (the `inkycap` folder under your system's usual place for application settings) and `.inkycap/plugins/` inside the notebox. Only `.json` files directly in those folders are read; subfolders are ignored.]

#callout("example")[A short manifest with a command that inserts a heading and a view, which lists your journal entries adds two `/` commands and one sidebar button the next time you open the notebox, with no rebuild required.]
