...
- *La collaboration est facultative, une boîte de notes à la fois.*
...
+ Là où votre modification et celle d'un collaborateur touchent les *mêmes lignes*, InkyCap garde la version *nouvellement fusionnée de votre collaborateur* et signale la note, au cas où vous voudriez annuler le changement ou comparer votre modification avec celle de votre collaborateur.
...
Le flux de travail lorsque vous recevez des changements : les changements entrent proprement, le rare chevauchement accepte la formulation de votre collaborateur, puis vous passez en revue ce qui est arrivé et vous annulez tout ce que vous préférez garder à votre façon.

#callout("note")[
  C'est voulu : InkyCap ne vérifie *pas* de lui-même les mises à jour de vos collaborateurs. Votre boîte de notes reste locale jusqu'à ce que _vous_ demandiez des mises à jour en cliquant sur *Synchroniser* ou *Vérifier les mises à jour*.
]

== Les deux façons de partager

Vous choisissez l'une de ces méthodes de synchronisation lors de la configuration. Bien qu'elles fonctionnent différemment en arrière-plan, à l'usage elles se comportent de la même manière une fois que vous travaillez (même fusion, même révision, même historique).

- *Synchronisation en ligne*. Votre boîte de notes se connecte à un dépôt partagé sur un service d'hébergement (comme Codeberg.org, GitHub ou tout serveur git de votre choix). Tout le monde se synchronise sur la même copie en ligne. Idéal pour une collaboration continue et en va-et-vient. #highlight[Pensez à rester attentif aux *considérations de confidentialité* et assurez-vous que vous ou vos collaborateurs configurez le dépôt en ligne de façon appropriée (certains peuvent rendre votre travail accessible au public)].
- *Échange de packages hors ligne*. Aucun serveur git n'est requis. Vous exportez la boîte de notes entière dans un seul fichier (un `.zip`), vous le transférez comme bon vous semble (par exemple une clé USB ou une pièce jointe de courriel), puis vous fusionnez le fichier d'un collaborateur lorsqu'il vous le retourne. Idéal quand vous ne voulez pas, ou ne pouvez pas, utiliser un serveur git en ligne, ou quand vous exigez plus de garanties de confidentialité.
...
- *Hors ligne (échange manuel de boîtes de notes)*. Laissez cette option désactivée pour la synchronisation git en ligne ; activez-la pour le transfert de packages par fichier. Vous pourrez passer à un serveur en ligne plus tard si vous commencez hors ligne.
- *Adresse du dépôt*. L'adresse web du dépôt partagé (pour la synchronisation en ligne), par exemple un lien `https://` que vous créez pour le dépôt git ou que votre collaborateur vous donne.
...
#callout("tip", title: "Configurations avancées")[
  Il existe un interrupteur *Avancé* « Se connecter avec SSH plutôt » qui vous connecte à l'aide des clés SSH existantes de votre machine au lieu d'un nom d'utilisateur et d'un mot de passe ; choisissez-le seulement si vous utilisez déjà SSH avec ce service. Les détails de connexion (le dépôt distant et la branche) résident par machine dans `.inkycap/local.json`, et _non_ dans le `settings.json` partagé. L'étiquette d'auteur se rabat sur le `git config` `user.name` / `user.email` de votre système, ce qui explique pourquoi les commits peuvent afficher un nom système.

L'interrupteur « `Regrouper les packages Typst lors du partage` » (désactivé par défaut) inclut tous les packages Typst additionnels installés dans la boîte de notes pour que les collaborateurs puissent compiler hors ligne ; les packages `@preview` se téléchargent automatiquement pour chaque personne.
]
...
Un cas apparenté : une boîte de notes configurée pour la collaboration, mais sans historique local des versions derrière elle (par exemple, une boîte copiée à la main sur un nouvel ordinateur plutôt que clonée, ou dont le dossier caché `.git` a été supprimé). Le volet Collaboration explique la situation et affiche le formulaire de configuration avec l'adresse de votre dépôt et votre branche déjà remplies ; le bouton au bas se lit *Réinitialiser le dépôt*. Choisissez-le pour reconstruire l'historique local et vous reconnecter.
...
- L'*pastille de la barre d'état* au bas de la fenêtre (une icône de poignée de main accompagnée d'un bref résumé d'état), qui apparaît _seulement sur les boîtes de notes où la collaboration est activée_.
...
Le volet affiche aussi une courte ligne d'état (par exemple « Aucune modification locale » quand vous êtes à jour, « Modifications à partager » quand vous avez du travail à envoyer, ou « 2 entrant(s) ») et un aperçu « Fichiers que vous partagerez » pour que vous voyiez exactement ce qu'une prochaine synchronisation transportera.
...
- Un *Mot de passe de l'archive* facultatif. Laissez-le vide pour aucun chiffrement, ou définissez-en un pour chiffrer le package (AES-256). Partagez ce mot de passe séparément.
...
Après chaque synchronisation ou importation de package, le volet Collaboration vous montre ce qui a changé.

=== Fichiers modifiés depuis la dernière synchronisation

C'est votre liste principale du « ce qui vient d'arriver ». Elle nomme chaque note dans laquelle la dernière synchronisation a intégré des changements (quand les changements proviennent d'un seul collaborateur, l'en-tête se lit « Changements fusionnés de » suivi de son nom). Pour chaque note, vous pouvez :
...
Ouvrez n'importe quelle note révisée et le volet *Modifications et historique* de la barre latérale droite affiche deux parties :

- *Reçues depuis la dernière synchronisation* montre les changements de votre collaborateur. Vous pouvez sauter à chaque changement dans la note, annuler un seul changement, ou *Tout rétablir* pour rétablir votre version complète d'avant la synchronisation.
...
En plus du partage, InkyCap offre deux façons de commenter un brouillon sans l'écraser. Elles sont incluses dans le processus de synchronisation.

- *Les annotations* sont des commentaires ou des questions qui ne changent pas le texte. Elles apparaissent sous forme de callout teinté à côté du contenu. Elles sont différentes des commentaires Typst en ligne qui commencent par `//` et qui ne sont pas visibles dans un résultat.
- *Les suggestions* sont des _changements_ proposés au contenu qui restent marqués comme en attente jusqu'à ce que quelqu'un les accepte ou les rejette. Il en existe trois sortes : suggérer une insertion, une suppression ou un remplacement.

Vous ajoutez l'une ou l'autre au texte que vous avez sélectionné, à l'aide de la palette de commandes (sous la catégorie « Édition ») ou des boutons au bas du volet des modifications :
...
Pour agir sur une suggestion, cliquez dessus (dans l'éditeur visuel, elle apparaît comme une petite pastille) pour ouvrir un menu *Accepter* / *Rejeter*, où vous pouvez aussi laisser un commentaire de suivi à son auteur.

#callout("important")[
  Accepter ou rejeter une suggestion la retire entièrement du texte. Une insertion acceptée devient du texte ordinaire, et une insertion rejetée disparaît. Cela signifie qu'un document fini que vous publiez ou exportez ne contient *pas* de marques de suggestion. Voyez #wikilink("3 - Exportation et publication").
]

== Historique des versions et restauration

Chaque note collaborative conserve son historique. Dans le volet *Modifications et historique*, passez de *Modifications* à *Historique* pour voir les versions antérieures, la plus récente en premier. L'entrée la plus récente porte le badge « actuelle » ou, si vous avez des modifications qui n'ont pas encore été partagées, « dernière partagée », accompagné d'une ligne expliquant que les versions listées sont l'historique partagé.

Pour une version antérieure, vous pouvez :
...
Si une synchronisation a déjà gardé la version d'un collaborateur par-dessus l'une des vôtres, l'entrée concernée porte le badge « votre version précédente (remplacée par la dernière synchronisation) », pour que vous puissiez trouver et restaurer ce que vous aviez écrit.
