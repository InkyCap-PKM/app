...
Chaque carte d'outil vous laisse décider comment le texte entre et sort.
...
- *Utiliser le résultat* est ce qu'InkyCap fait avec ce que le programme affiche :
  ...
  - « L'afficher dans un volet latéral » ouvre un panneau durable à droite (à côté de Propriétés, Plan et Liens) avec les résultats de l'outil. Idéal pour les rapports plus longs. Le panneau est rattaché à la note sur laquelle vous l'avez lancé, et relancer l'outil rafraîchit le même panneau.
...
- *Afficher dans* est l'endroit où vous accéderez à l'outil :
  ...
  - « Menu / de l'éditeur » le place dans le menu `/` de l'éditeur. Taper `/` remplace votre sélection au fur et à mesure, ce qui convient aux outils qui insèrent du texte.
  - « Les deux ».

- *Envoyer le texte brut seulement* est activé par défaut. Avec ceci activé, InkyCap retire le balisage Typst, les propriétés, le math et le code avant l'envoi, afin que le programme ne reçoive que votre prose. C'est idéal pour les correcteurs grammaticaux et stylistiques. Désactivez-le si votre outil a besoin de la source brute.

#callout("tip")[
  Choisissez « Palette de commandes » + « Remplacer la sélection » pour les aides à la révision (grammaire, réécriture), et « L'afficher dans un volet latéral » pour les outils qui produisent un rapport que vous voulez continuer à lire. Si un outil ne fait que répondre à une question rapide, « Afficher un message temporaire » garde les choses bien rangées.
]

=== Choses à savoir sur la sécurité et les limites

- *La vie privée reste entre vos mains.* InkyCap n'envoie vos notes nulle part de lui-même. Si un outil que vous avez configuré communique avec un service réseau, c'est votre choix ; le texte de votre note va là où vous l'avez pointé.
...
- *Pas de surprises.* InkyCap exécute le programme que vous avez enregistré, identifié à l'interne, et il ne fait pas passer les commandes par un shell.
...
#callout("tip", title: "Détails")[
  ...
  $INKYCAP_SELECTION      le texte sélectionné (toujours brut, même quand
                          « Envoyer le texte brut seulement » est activé)
  ```

  Pour un programme qui lit son entrée depuis les arguments plutôt que depuis un flux, réglez *Envoyer à l'outil* sur « Rien » et mettez `$INKYCAP_SELECTION` (ou `$INKYCAP_FILE`) dans *Arguments*. Le retrait vers du texte brut utilise un extracteur basé sur l'AST qui suit l'analyseur Typst. Les utilisateurs avancés peuvent aussi modifier `external_tools.tools` directement dans `settings.json`.
]
...
#callout("tip", title: "Détails")[
  Les deux dossiers qu'InkyCap lit sont `plugins/` à l'intérieur de son dossier de configuration (le dossier `inkycap` sous l'emplacement habituel de votre système pour les paramètres d'applications) et `.inkycap/plugins/` à l'intérieur de la boîte de notes. Seuls les fichiers `.json` placés directement dans ces dossiers sont lus ; les sous-dossiers sont ignorés.
]
