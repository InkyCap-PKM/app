// Makes the inkycap-notebox `#import` line uneditable in the source editor.
//
// The line stays visible — it's useful context, and revealing it is better
// than hiding notebox machinery — but it must not be casually altered or
// deleted: every note auto-imports the notebox package through it, and a
...
// `externalReload` (file open, sidebar property reload, notebox upgrade) pass
// through untouched.

import { EditorState } from "@codemirror/state";
import { isNoteboxImportLine, externalReload } from "./visual-protected";

/** Byte range of the inkycap-notebox import line including its trailing
...
function importLineRange(
  ...
  for (let i = 1; i <= maxScan; i++) {
    ...
    if (!isNoteboxImportLine(line.text)) continue;
    const to =
...
/** Change filter that protects the inkycap-notebox `#import` line from edits. */
